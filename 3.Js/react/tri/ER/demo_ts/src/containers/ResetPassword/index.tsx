...
class ResetPassword extends React.Component<Props, State> {
    ...
    componentDidMount = async () => {
        const checkSecretKey = await API.postApi("check-reset-pass-secret-key", {
            resetPassSecretKey: this.state.resetPassSecretKey
        })

        if (checkSecretKey.status === 200 && checkSecretKey.data.http_code !== 200) {
    ...
    handleSubmit = async () => {
        ...
        const validPasswordNew = validate.validPasswordStrong(newPassword),
            validComfirm = validate.validPasswordComfirm(newPassword, confirmPassword),
            paramUser = {
                resetPassSecretKey,
                newPassword
            };
        ...
        if (newPassword.length > 0 && confirmPassword.length > 0 && validPasswordNew && validComfirm) {
            ...
            error = {
                ...
                confirmPassword: confirmPassword.length === 0 ? Constant.MESSAGE_CODE["006"] : (!validComfirm && validPasswordNew ? Constant.MESSAGE_CODE["007"] : "")
            }
        }
