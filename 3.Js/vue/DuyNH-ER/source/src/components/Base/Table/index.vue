<template>
  <div>
    <table class="table-comp table-bordered mb-4">
      ...
      <tbody>  
        <tr v-for="(item, index) in items" :key="index" @click="valueRowSelected(item)" :class="{'selected': (item == selectedItem)}">     
          <td v-for="(td, num) in fields " :key="num" >
            <span>{{ td.key === 'id'  ? index + 1 : td.key === 'active' ? ''  : item[td.key]}}</span>  
            <b-form-checkbox v-if="td.key === 'active'"
              v-model="item[td.key]" value="1" unchecked-value="0" disabled>
            </b-form-checkbox>
          </td>  
        </tr>
      </tbody>
